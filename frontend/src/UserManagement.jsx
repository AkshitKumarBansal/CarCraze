          ...
          <tbody>
            {users.map(user => (
              <tr key={user._id}>
                ...
                <td>
                  {user.role === 'seller' ? (
                    <span className={`status-badge ${user.verification?.status === 'approved' ? 'active' : 'inactive'}`} style={{textTransform: 'capitalize'}}>
                      {user.verification?.status || 'unverified'}
                    </span>
                  ) : (
                    <span className="status-badge" style={{backgroundColor: '#f3f4f6', color: '#6b7280'}}>N/A</span>
                  )}
                </td>
